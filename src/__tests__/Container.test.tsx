...
describe('container-test', () => {
	...
	it('should render list of images', async () => {
		render(<Container />);
		const imagesData = await screen.findAllByTestId('image-list');
	...
	it('should render two button', () => {
		...
		const buttons = screen.getAllByTestId('button-test');
	...
	it('clicking button', async () => {
		...
		const buttons = screen.getByText('Prev');
		...
		const imagesDat = await screen.findAllByTestId('image-list');
		expect(imagesDat.length).toBe(1);
	});
});
